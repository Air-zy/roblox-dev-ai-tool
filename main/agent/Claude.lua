--!optimize 2
-- Claude.luau — Anthropic Messages API client for plugin use
--
-- Calls https://api.anthropic.com/v1/messages using the OAuth access token
-- obtained from OAuth.luau. Sent via `Authorization: Bearer`.
--
-- IMPORTANT — Claude Code rate-limit pool detection:
-- Anthropic routes OAuth-token requests to different rate-limit pools based on
-- whether the request "looks like" it came from the official Claude Code CLI.
-- The signals Anthropic checks:
--   1. anthropic-beta header includes `claude-code-20250219,oauth-2025-04-20`  ✅
--   2. user-agent looks like `claude-cli/X.Y.Z (external, cli)`               ❌ BLOCKED by Roblox
--   3. x-app: cli                                                              ✅
--   4. system[0] is exactly "You are Claude Code, Anthropic's official CLI…"   ✅
--
-- ROBLOX LIMITATION: HttpService locks `User-Agent` and `Roblox-Id` headers
-- (per create.roblox.com/docs/reference/engine/classes/HttpService). We cannot
-- send `user-agent: claude-cli/...`. Roblox sends its own User-Agent.
-- If Anthropic's detection strictly requires the claude-cli UA, we'll stay in
-- the tight third-party rate-limit pool. The other 3 signals may be enough —
-- needs empirical testing.
--
-- NOTE on signal 4: that identity block is a billing artefact, not a
-- description of this program. It tells the model it is the Claude Code CLI,
-- which ships a Bash tool over a real filesystem — so Settings.system() has to
-- state up front that there is no disk here and that `bash` operates on the
-- DataModel, or the model reasons from the wrong prior about what its tools do.
--
-- Public API:
--   Claude.Initialize(oauthModule)
--   Claude.streamMessage({ model=, system=, messages=, maxTokens=, tools= }, callbacks) -> handle
--   Claude.webSearchTool(maxUses)
--   Claude.MODELS
--
-- The non-streaming sendMessage/sendWithTools pair that used to live here is
-- gone: Agent.luau drives the tool loop over streamMessage, and nothing had
-- called either function since. Two implementations of one protocol is how the
-- sendToClaude/sendToClaude_continue drift happened in the first place.
...
-- Beta header values that mark the request as Claude Code traffic.
local ANTHROPIC_BETA = "interleaved-thinking-2025-05-14,fine-grained-tool-streaming-2025-05-14,claude-code-20250219,oauth-2025-04-20"
